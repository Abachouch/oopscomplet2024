import  { useState, ChangeEvent, useEffect } from "react"
import caleforniaCities from '../data/calefornia_cities.json';

export default function CityPicker({ selectedCity, onCitySelected }: {selectedCity:string  , onCitySelected: (city:string) => void }) {

    const [inputValue, setInputValue] = useState<string>("");
    const [suggestions, setSuggestions] = useState<string[]>([]);

    useEffect(()=>{
      setInputValue(selectedCity)
    } , [selectedCity])

    const handleInputChange = (event: ChangeEvent<HTMLInputElement>) => {
        const newValue = event.target.value;
        setInputValue(newValue);
       // Show suggestions only when typing
       if (newValue.trim() !== '') {
        // Filter cities based on input
        const filteredCities = caleforniaCities.filter((city) =>
          city.toLowerCase().includes(newValue.toLowerCase())
        );
        // Display up to 5 suggestions
        setSuggestions(filteredCities.slice(0, 5));
      } else {
        setSuggestions([]);
      }
      onCitySelected(newValue) ;
    };
    
    const handleSuggestionClick = (city: string) => {
        setInputValue(city);
        onCitySelected(city) ;
        setSuggestions([]);
    };

    return (
        <div className="citypicker-wraper">
            <input value={inputValue} onChange={handleInputChange} aria-label='location' required placeholder="Location" type="text" name="location" id="location" />
            {suggestions.length > 0 && (
            <ul className="citypicker-list">
                {suggestions.map((city, index) => (
                    <li className="citypicker-item" key={index} onClick={() => handleSuggestionClick(city)}>
                        <div className="citypicker-icon">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <mask id="mask0_347_2208" style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="24">
                                    <rect width="24" height="24" fill="#D9D9D9" />
                                </mask>
                                <g mask="url(#mask0_347_2208)">
                                    <path d="M12 12C12.55 12 13.0208 11.8042 13.4125 11.4125C13.8042 11.0208 14 10.55 14 10C14 9.45 13.8042 8.97917 13.4125 8.5875C13.0208 8.19583 12.55 8 12 8C11.45 8 10.9792 8.19583 10.5875 8.5875C10.1958 8.97917 10 9.45 10 10C10 10.55 10.1958 11.0208 10.5875 11.4125C10.9792 11.8042 11.45 12 12 12ZM12 19.35C14.0333 17.4833 15.5417 15.7875 16.525 14.2625C17.5083 12.7375 18 11.3833 18 10.2C18 8.38333 17.4208 6.89583 16.2625 5.7375C15.1042 4.57917 13.6833 4 12 4C10.3167 4 8.89583 4.57917 7.7375 5.7375C6.57917 6.89583 6 8.38333 6 10.2C6 11.3833 6.49167 12.7375 7.475 14.2625C8.45833 15.7875 9.96667 17.4833 12 19.35ZM12 22C9.31667 19.7167 7.3125 17.5958 5.9875 15.6375C4.6625 13.6792 4 11.8667 4 10.2C4 7.7 4.80417 5.70833 6.4125 4.225C8.02083 2.74167 9.88333 2 12 2C14.1167 2 15.9792 2.74167 17.5875 4.225C19.1958 5.70833 20 7.7 20 10.2C20 11.8667 19.3375 13.6792 18.0125 15.6375C16.6875 17.5958 14.6833 19.7167 12 22Z" fill="#1C1B1F" />
                                </g>
                            </svg>
                        </div>
                        <div className="citypicker-info">
                            <div className="citypicker-city">{city}</div>
                            <div className="citypicker-country">United States</div>
                        </div>
                    </li>
                ))}
            </ul>
            )}
        </div>
    )
}